...
export function SectionSettings({ section, onSettingsChange, onToggleEnabled }: SectionSettingsProps) {
  ...
  const isFooter = String(section?.name || section?.type || '').toLowerCase().includes('footer')
        ...
        {isFooter && (
          <div className="mt-3 p-3 rounded border border-gray-200 bg-gray-50">
            <p className="text-xs text-gray-600">
              The footer is hardcoded and inherits background color from the header. Footer content is not editable here.
            </p>
          </div>
        )}
      </div>

      {/* Visibility Toggle (hidden for Footer) */}
      {!isFooter && (
