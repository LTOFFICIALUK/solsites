...
export default function ProductionPreviewPage() {
    ...
    const loadProject = async () => {
      try {
        ...
        const parsedData = (project?.data as ProjectData) || ({
          ...
          template: 'neon',
          colors: { primary: '#FF0080', secondary: '#00FFFF', accent: '#FFD700' },
          content: { hero: { title: 'Happy PEPE', subtitle: 'The viral frog memecoin', description: 'PEPE is taking over crypto with memes, community, and unstoppable vibes.' }, about: { title: 'Token Details', content: 'PEPE is the community-driven meme phenomenon.' } } as any
        } as any)
  ...
  const transformProjectData = (data: ProjectData) => {
    // Prefer block content when present, like the editor does
    const allBlocks = (sections || []).flatMap((s: any) => s?.blocks || [])
    const getBlockContent = (match: (t: string) => boolean) => {
      const blk = allBlocks.find((b: any) => match(((b?.type || b?.name || '') as string).toLowerCase()))
      return (blk && (blk as any).content) || {}
    }
    const heroBlockContent = getBlockContent(t => t.includes('hero'))

    const normalizedTeamMembers = ((data as any)?.content?.team?.members || [
    ...
    return {
      ...
      content: {
        hero: {
          // Align Neon defaults with the Neon component to avoid mismatches
          title: (heroBlockContent as any).title || (data as any)?.content?.hero?.title || 'Happy PEPE',
          subtitle: (heroBlockContent as any).subtitle || (data as any)?.content?.hero?.subtitle || 'The viral frog memecoin',
          description: (heroBlockContent as any).description || (data as any)?.content?.hero?.description || 'PEPE is taking over crypto with memes, community, and unstoppable vibes.',
          showStats: ((heroBlockContent as any).showStats !== undefined ? (heroBlockContent as any).showStats : ((data as any)?.content?.hero?.showStats ?? true)),
          stats: (heroBlockContent as any).stats || (data as any)?.content?.hero?.stats || {
            holders: '10,000+',
            marketCap: '$2.5M',
            volume24h: '$500K',
          },
          primaryButton: (heroBlockContent as any).primaryButton || (data as any)?.content?.hero?.primaryButton || { text: 'Buy PEPE', href: '' },
        },
        about: {
        ...
        team: {
          title: (data as any)?.content?.team?.title,
          description: (data as any)?.content?.team?.description,
